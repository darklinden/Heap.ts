...
export default class Heap<T> {
    ...
    of a to be searched.
     */
    private insort<T>(array: T[], x: T, low: number, high: number, cmp: (x: T, y: T) => number = defaultCmp): void {
     ...
     */
    private _heappop<T>(array: T[], cmp: (x: T, y: T) => number = defaultCmp): T {
    ...
    private nsmallest<T>(array: T[], n: number, cmp: (x: T, y: T) => number = defaultCmp): T[] {
        ...
        for (let i = 0; i < Math.min(n, array.length); i++) {
            result.push(this._heappop(array, cmp));
        }
        return result;
    }
    ...
    private _cmp: (x: T, y: T) => number = null;

    constructor(cmp: (x: T, y: T) => number = null) {
        this._cmp = cmp != null ? cmp : defaultCmp;
        this._nodes = [];
    }
